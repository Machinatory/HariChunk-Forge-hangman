...
public class FindTopSurfaceNode implements AstNode {
    ...
    public double evalSingle(int x, int y, int z, EvalType type) {
        int topCellBlockY = InvocationShim.invokeFloor(this.upperBound.evalSingle(x, y, z, type) / this.cellHeight) * this.cellHeight;
    ...
    public void evalMulti(double[] res, int[] x, int[] y, int[] z, EvalType type) {
        ...
        for (int i = 0; i < res.length; i++) {
            topCellBlockY[i] = InvocationShim.invokeFloor(res[i] / this.cellHeight) * this.cellHeight;
        }

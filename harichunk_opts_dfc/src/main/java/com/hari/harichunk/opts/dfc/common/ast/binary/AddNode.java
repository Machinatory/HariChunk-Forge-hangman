...
public class AddNode extends AbstractBinaryNode {
    public AddNode(AstNode left, AstNode right) { super(left, right); }
    @Override protected AstNode newInstance(AstNode left, AstNode right) { return new AddNode(left, right); }
    ...
    @Override protected void bytecodeGenMultiBody(InstructionAdapter m, int idx, int res1) {
        ...
        m.add(Type.DOUBLE_TYPE); m.astore(Type.DOUBLE_TYPE);
    }
}

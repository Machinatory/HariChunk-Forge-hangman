...
public class DFTShiftBNode implements AstNode {
    ...
    public double evalSingle(int x, int y, int z, EvalType type) {
        return InvocationShim.invokeNoiseHolderSample(this.offsetNoise, z * 0.25, x * 0.25, 0.0) * 4.0;
    }

    @Override
    public void evalMulti(double[] res, int[] x, int[] y, int[] z, EvalType type) {
        for (int i = 0; i < res.length; i++) {
            res[i] = InvocationShim.invokeNoiseHolderSample(this.offsetNoise, z[i] * 0.25, x[i] * 0.25, 0.0) * 4.0;
        }
    }

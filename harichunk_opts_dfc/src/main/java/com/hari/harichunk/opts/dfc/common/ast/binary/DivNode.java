...
public class DivNode extends AbstractBinaryNode {
    public DivNode(AstNode left, AstNode right) { super(left, right); }
    @Override protected AstNode newInstance(AstNode left, AstNode right) { return new DivNode(left, right); }
    ...
    @Override public void doBytecodeGenSingle(BytecodeGen.Context context, InstructionAdapter m, BytecodeGen.Context.LocalVarConsumer localVarConsumer) {
        ...
        m.div(Type.DOUBLE_TYPE); m.areturn(Type.DOUBLE_TYPE);
    }
    @Override protected void bytecodeGenMultiBody(InstructionAdapter m, int idx, int res1) {
        ...
        m.div(Type.DOUBLE_TYPE); m.astore(Type.DOUBLE_TYPE);
    }
}

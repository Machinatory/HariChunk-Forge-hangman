...
import com.hari.harichunk.opts.dfc.common.ast.dfvisitor.StripBlending;
...
public class BytecodeGen {
    ...
    private static final File exportDir = new File("./cache/harichunk-dfc");
    ...
    public static synchronized CompiledEntry compile0(AstNode node) {
        ...
        GpuDensityProgramCompiler.EncodedProgram gpuProgram = GpuDensityProgramCompiler.compile(node);
        ...
        writer.visit(Opcodes.V17, Opcodes.ACC_PUBLIC | Opcodes.ACC_FINAL, name, null, Type.getInternalName(Object.class), new String[]{Type.getInternalName(CompiledEntry.class)});
        ...
        if (cached != null) {
            try {
                return GpuCompiledEntry.wrap((CompiledEntry) cached.getConstructor(List.class).newInstance(args), gpuProgram);
        ...
        while (iterator.hasNext()) {
            ...
            // No DensityFunctionTypes.Wrapping in 1.20.1; equivalent is DensityFunctions.Marker
        }
        ...
        try {
            return GpuCompiledEntry.wrap((CompiledEntry) defined.getConstructor(List.class).newInstance(args), gpuProgram);
        } catch (InstantiationException | IllegalAccessException | InvocationTargetException | NoSuchMethodException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    public static class Context {
        ...
        private final Reference2ReferenceOpenHashMap<Object, String> cachedSplineMethods = new Reference2ReferenceOpenHashMap<>();
        ...
        public String getCachedSplineMethod(CubicSpline<DensityFunctions.Spline.Point, DensityFunctions.Spline.Coordinate> spline) {
            return this.cachedSplineMethods.get(spline);
        }

        public void cacheSplineMethod(CubicSpline<DensityFunctions.Spline.Point, DensityFunctions.Spline.Coordinate> spline, String methodName) {
            this.cachedSplineMethods.put(spline, methodName);
        }
